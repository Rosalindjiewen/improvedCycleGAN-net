...
class CycleGAN():
    def __init__(self):
        ...
        # 载入数据
        self.dataset_name = 'healthy2disease'
    ...
    def train(self, init_epoch, epochs, batch_size=1, sample_interval=50):
        ...
        for epoch in range(init_epoch,epochs):
            ...
            for batch_i, (imgs_A, imgs_B) in enumerate(self.data_loader.load_batch(batch_size)):
                # ------------------ #
                #  训练生成模型：input：真实图像A和B；output：六个loss
                # ------------------ #
                ...
                #output:
                output_txt = "[Epoch %d/%d] [Batch %d/%d] [D loss: %f, acc: %3d%%] [G loss: %05f, adv: %05f, recon: %05f, id: %05f] time: %s " \
                                                                        % ( epoch, epochs,
...
if __name__ == '__main__':
    gan = CycleGAN()
    gan.train(init_epoch=0, epochs=200, batch_size=4, sample_interval=200)
